...
const ColorPickerPro: React.FC<ColorPickerProProps> = memo(({
  isOpen,
  onClose,
  onColorSelect,
  initialColor = '#0066CC',
  ...
  // 处理HSV颜色变化
  const handleHSVColorChange = useCallback((color: string) => {
  ...
  return (
    <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-4">
      <div
        ref={pickerRef}
        className={`glass-premium rounded-2xl w-full max-w-lg mx-4 animate-scale-in shadow-2xl ${className}`}
      ...
      >
        ...
        <div className="flex items-center justify-between p-5 pb-0">
          <div className="flex items-center space-x-2">
            ...
            <h3 className="text-lg font-semibold text-glass-primary">专业取色板</h3>
          </div>
          <button
            onClick={onClose}
          ...
          <div className="flex space-x-1 glass-secondary rounded-lg p-1">
            <button
              ...
              className={`flex-1 px-4 py-2 text-sm font-medium rounded-md transition-all duration-200 ${
                activeTab === 'picker'
                  ? 'glass-button text-glass-primary shadow-sm'
...
ColorPickerPro.displayName = 'ColorPickerPro';
